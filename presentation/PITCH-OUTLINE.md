...
- 設計変更は詳細設計だけでなく、商品企画、パッケージ設計、CAE、試作評価、生産技術、金型・治具、調達、品質保証、法規、市場品質など、開発から量産後まで多くの工程で発生する。
...
- 2015 → 2020 の再発タイムライン
- フランジの板厚変化、再発点を赤く表示
- 設計変更が発生しうる工程のミニマップ（企画 / パッケージ / 詳細設計 / CAE / 試作評価 / 生産技術 / 金型治具 / 調達 / 品質 / 法規 / 市場品質）を中央に置き、「変更の入口が多すぎる」ことを視覚化する
- その工程マップの中で、軽量化・原価・調達・加工性・品質・法規などの理由が混ざり、過去の「なぜ」が埋もれることを示す
...
- `CoLab reviews formal rules.`
- `Leo retrieves knowledge.`
- `CADDi finds drawings.`
- `SenpAI watches the edit, reasons with evidence, and writes the lesson back.`
- 1行比較: `Search finds documents. SenpAI catches risky design decisions.`
...
- **TAM: 約3,500億〜7,000億円 / 年**
- 算出: 自動車SAMを、日本の自動車製造業が全製造業出荷額に占める比率 **19.2%** で割り戻す。
- 根拠: JAMAは2023年の自動車製造品出荷額を **71兆5,991億円**、全製造業に占める割合を **19.2%** としている。
...
- **SAM: 約670億〜1,340億円 / 年**
- 算出: 自動車関連就業人口 **559万人** × 対象技術者比率 **2〜4%** × 年間単価 **60万円/seat**。
- 対象技術者比率2〜4%は、製品設計・生産技術・金型設計・品質保証など「設計判断に関わる席」に絞った仮説。
- 根拠: JAMAは自動車関連産業の就業人口を **559万人** としている。
...
- **SOM: 約0.6億〜7.2億円 ARR**
- 算出: 初期商用 **5〜20部署** × 1部署あたり年間契約額 **1,200万〜3,600万円**。
- 1部署あたり年間契約額: **20〜60席 × 5万円/月 × 12ヶ月**。
- PoCは1部署・1部品群から開始し、再発防止の実績を作って部門横展開する。

**再発1件あたりコスト仮説**:

- **約2,000万円〜2億円 / 件**
- 内訳: 設計手戻り工数、レビュー再実施、試作・評価やり直し、サプライヤー調整、量産遅延リスク。
- 1部署の年間契約額1,200万〜3,600万円は、「再発1件を防げば回収できる」価格帯に置く。
...
- JAMA「基幹産業としての自動車製造業」: 2023年自動車製造品出荷額71兆5,991億円、全製造業比19.2%、自動車関連産業就業人口559万人。
  https://www.jama.or.jp/statistics/facts/industry/
- World Bank API `NV.IND.MANF.CD`: 日本の製造業付加価値は2024年に約7,884億米ドル。
  https://api.worldbank.org/v2/country/JPN/indicator/NV.IND.MANF.CD?format=json
...
- [x] TAM/SAM/SOMの外部ソース調査と数字入力
- [x] 1部署あたり年間契約額の仮説を決める
- [x] 再発1件あたりのコストレンジを、手戻り工数・試作遅延・レビュー工数で見積もる
- [x] 競合比較の表現を英語スライド用に短文化
- [x] `presentation/index.html` または `presentation/pitch-b.html` を新構成へ反映
- [ ] 90秒デモの位置はSlide 4直後に固定済み。リハーサルで時間配分を確認
...
- TAM/SAM/SOMの数字はJAMA等の外部ソースを起点にした仮説値。本番前に実顧客ヒアリングでSOM前提を再確認する。
